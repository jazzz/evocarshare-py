...
EPSILON = 0.005  # Allow 1/2 percent variance when comparing gps distances
...
class TestCoord:
    ...
    VAN_10m = GpsCoord(49.27979984695, -123.10212590000)
...
def test_gps_ident():
    ...
    assert TestCoord.NULL_ISLE.distanceTo(TestCoord.NULL_ISLE_1k) == pytest.approx(1000, rel=EPSILON)
    assert TestCoord.VAN_10m.distanceTo(TestCoord.VAN) == pytest.approx(10, abs=EPSILON)
...
def test_ranged_vehicle_attr():
    ...
    assert r.distance == pytest.approx(1000, rel=EPSILON)
